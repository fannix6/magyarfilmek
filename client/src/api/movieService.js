...
export default {
  ...
  async getAllSortSearch(column = "id", direction = "asc", search = "") {
    const route = `/moviessortsearch/${column}/${direction}/${search}`;
    return await apiClient.get(route);
  },
  ...
  async getPaging(
    ...
    let route = `/moviespaging/${page}/${per_page}/${column}/${direction}`;
    if (search) {
      route = `/moviespaging/${page}/${per_page}/${column}/${direction}/${search}`;
    }
    return await apiClient.get(`${route}`);
  },
