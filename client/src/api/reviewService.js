...
export default {
  ...
  async getAllSortSearch(column = "id", direction = "asc", search = "") {
    const route = `/reviewssortsearch/${column}/${direction}/${search}`;
    return await apiClient.get(route);
  },
  ...
  async getPaging(
    ...
    let route = `/rewievspaging/${page}/${per_page}/${column}/${direction}`;
    if (search) {
      route = `/rewievspaging/${page}/${per_page}/${column}/${direction}/${search}`;
    }
    return await apiClient.get(`${route}`);
  },
